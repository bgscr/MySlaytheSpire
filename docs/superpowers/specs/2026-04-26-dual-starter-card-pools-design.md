...
在已完成的 Phase 2 内容承载基础上，为剑修和丹修各新增 8 张基础卡牌，让两个角色的卡池从示例状态进入可测试的小型内容池。
...
| `sword.cloud_step` | 云身步 | 1 | `skill` | `common` | 获得 4 护体，抽 1 张牌 |
| `sword.focused_slash` | 凝神斩 | 1 | `attack` | `common` | 造成 8 伤害 |
| `sword.sword_resonance` | 剑鸣入体 | 1 | `skill` | `uncommon` | 获得 3 护体，获得 2 层 `sword_focus` |
| `sword.horizon_arc` | 横天剑弧 | 2 | `attack` | `uncommon` | 造成 6 伤害，获得 4 护体 |
...
| `alchemy.calming_powder` | 定神散 | 1 | `skill` | `common` | 获得 6 护体，回复 2 生命 |
| `alchemy.toxin_needle` | 毒针入脉 | 1 | `attack` | `common` | 造成 3 伤害，施加 2 层 `poison` |
| `alchemy.spirit_distill` | 灵液萃取 | 1 | `skill` | `uncommon` | 抽 2 张牌 |
| `alchemy.cinnabar_seal` | 朱砂护印 | 2 | `skill` | `uncommon` | 获得 8 护体，获得 1 层 `elixir_guard` |
...
后续关卡不应假设每次遭遇只有一个敌人。当前 `CombatState.enemies` 已经使用数组承载敌人列表，但 `EncounterGenerator` 暂时只返回单个敌人 id，`CombatScreen` 也仍是占位流程。

本批卡牌不实现多敌人战斗 UI 或敌人轮流行动。所有敌方目标效果都作用于一个已选择目标，避免在卡牌资源中写入单敌人假设。多敌人遭遇生成、敌人轮流进攻和多目标表现应在后续敌人池/遭遇扩展计划中单独实现。
...
- `tests/unit/test_content_catalog.gd`
  - 默认 catalog 加载 18 张卡牌。
  - 剑修卡池包含 9 张，丹修卡池包含 9 张。
  - 两个卡池互不包含对方卡牌。
...
- `tests/unit/test_reward_generator.gd`
  - 剑修卡牌奖励只从剑修卡池产生。
  - 丹修卡牌奖励只从丹修卡池产生。
  - 请求 3 张卡时，在当前 9 张角色卡池中返回 3 张不重复卡。

- `tests/unit/test_combat_engine.gd`
  - 新增多效果卡牌可通过现有 `play_card_in_state()` 承载。
  - 覆盖一张攻击加抽牌卡和一张能量加抽牌卡，避免资源效果组合误配。
  - 不增加群体攻击或随机目标测试；多敌人目标选择留给后续遭遇实现。

## 验收标准

完成后应满足：

- 默认 `ContentCatalog` 加载 18 张卡牌、2 个角色、3 个敌人和 1 个遗物。
- 剑修和丹修各有 9 张可查询卡牌。
