...
- `assets/presentation/card_thumbnails/*.png.import` for the 40 new PNGs, if Godot creates them.
...
- [ ] **Step 1: Update representative catalog expectations**
...
- [ ] **Step 2: Add unique polished thumbnail path test**
...
func test_default_catalog_card_visuals_use_unique_polished_thumbnail_paths() -> bool:
	...
	var passed: bool = seen_paths.size() == 40
	assert(passed)
	return passed
```

- [ ] **Step 3: Update representative resolver expectations**
...
- [ ] **Step 5: Keep failing guardrails uncommitted**
...
- [ ] **Step 2: Generate source art with the built-in image generation tool**

Use the built-in image generation path, one generated image per card. Save or copy the selected generated output for each card to:

```text
tmp/card_art_sources/<filename>.png
```

Use this shared prompt prefix for every generated card:
...
- [ ] **Step 3: Normalize generated sources to 128x96 project thumbnails**

After all 40 source images exist in `tmp/card_art_sources/`, run this crop-and-resize command:
...
- [ ] **Step 4: Verify all expected PNGs exist and have 128x96 dimensions**
...
- [ ] **Step 5: Commit generated normalized PNGs**
...
```powershell
rtk git add tests/unit/test_content_catalog.gd tests/unit/test_combat_visuals.gd resources/visuals/card_visuals/*.tres assets/presentation/card_thumbnails/*.png.import docs/superpowers/plans/2026-05-10-polished-per-card-art-replacement.md
rtk git commit -m "feat: point card visuals to polished thumbnails"
```

If no `.png.import` files were generated, omit `assets/presentation/card_thumbnails/*.png.import`.
